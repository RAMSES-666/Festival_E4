...
class FestivalController extends Controller
{
    ...
    public function AffectAtrib(){
        $idEquipe = $_POST["AttribEkip"];
        $nombreChambres = $_POST["nombreChambresDesiree"];
        $idEtab = $_POST["AttribEtab"];

        $reqEtab = DB::select('select idEtablissement from Etablissement WHERE nom ="'.$idEtab.'"');

        DB::insert("insert into Attribution values ('$idEtab', '$idEquipe', '$nombreChambres')");

        return view('Attribution');
    }

        public function obtenirModifAttribution(){
        $reqModif = DB::select('select * from Attribution where idEquipe="'.$_GET['idEquipe'].'" AND idEtab="'.$_GET['idEtab'].'"');
        return view('modificationAttributions', compact('reqModif'));
    } 
}
